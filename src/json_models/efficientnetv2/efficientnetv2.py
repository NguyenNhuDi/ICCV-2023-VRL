...
class XModuleStateController:
    def __init__(self):
        assert False, "Don't instantiate this"
    @classmethod
    def get_xmodule_kernel_dilation(cls):
        return cls.kernels, cls.dilations
    @classmethod
    def set_xmodule_kernel(cls, kernels: list[int]):
        cls.kernels = kernels
        cls.dilations = [1] * len(kernels)
...
class Conv(torch.nn.Module):
    def __init__(self, in_ch, out_ch, activation, k=1, s=1, g=1):
        ...
        if g > 1:
            ...
            kernel, dilation = XModuleStateController.get_xmodule_kernel_dilation()
            self.conv = XModule(in_ch, out_ch, kernel_sizes=kernel, dilations=dilation)
...
class EfficientNetv2(nn.Module):
    ...
    def __init__(self, drop_rate=0, num_class=1000, kernel_sizes=[7]):
        super().__init__()
        num_dep = [3, 5, 5, 7, 14, 18, 5]
        filters = [24, 48, 80, 160, 176, 304, 512, 1280]
        XModuleStateController.set_xmodule_kernel(kernel_sizes)
